...
class fi{
	public:char file_name[30][30];
};
fi file_n[100000];

using namespace std;
void command(int* client,int maxi,int id,char *recv_line,int sockfd)
{
    ...
    if(strcmp(opcode,"CLIENTID") == 0)
    {
    	...
    	for(int i=10;i<15;i++)
    	{
    		temp_id[counter] = recv_line[i];
    		counter++;
		}
		temp_id[counter]='\0';
		int temp = atoi(temp_id);
		clientid[id] = temp;
	}
    else  if(strcmp(opcode,"PUT") == 0)//op 0
    {
		op = 0;
		...
		fclose(fp);
		int temp=clientid[id];
		strcpy(file_n[temp].file_name[list_count[temp]],data_2[id]);
		list_count[temp]++;
    }
    else if(strcmp(opcode,"LIST")== 0)//op 1
    {
		op = 1; 
		int temp = clientid[id];
		for(int i=0;i<list_count[temp];i++)
		{
			sprintf(buffer,"%s\n",file_n[temp].file_name[i]);
...
int main(int argc, char** argv)
{
    ...
    while(1)
    {
        ...
        for(int j= 0;j<=maxi;j++)
        {
			...
			command(client, maxi, j,recv_line,sockfd);
